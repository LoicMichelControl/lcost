    ...
    %%
    %%
    %%  LCO/St - Lift Control OpenFAST & Stabilization / Main Module - Dec 2023 - March 2026
    %%                                   v1.8.3b
    ...
    % Warning : concerning the stochastic cases 'sto',
    % please update the name of the zip file (TurbSim .bts files) to be
    % unzipped:
    %TurbSim_zip_name = 'TurbSim_undersampled_profile_13_18.zip';
    

        % Example A
        % path = '../Examples/A_OpenLoop/';
        ...
        % Example ComputeSurface
        % path = '../Examples/ComputeSurface/';
    

    % B/ other simulation cases (v1.8.0)
    ...
    fprintf('\n\n === LCO/St - Lift Control OpenFAST & Stabilization / Main Module - v1.8.3b (Apr. 2026) === \n\n')
    ...
    unzip('../lib_OpenFAST_Sequoia.zip', '');
    fprintf("Unzip librairies... done. \n")
    ...
    % -------------------------------------------
    % Create the directory to save the customized
    % input files of OpenFAST
    
    cd(LCOS_rootFolder)
    cd(path)
    
    if ( exist('OFastFiles','dir') == 0)
        mkdir OFastFiles
    
        cd OFastFiles
        le_input_files_work = dir('_*');
        fprintf('Create the OFastFiles dir. in the working dir. \n');
    else
        ...
        if ( length(le_input_files_work) == 5 )
            fprintf('Check modified files in the working dir. : %d files -> OK \n', length(le_input_files_work))
        else
            fprintf('Check modified files in the working dir. : missing files ! \n');
        end
    ...
    if ( isempty(FSTFound) == 0  )
        ...
        Check_FAST_OutFile = ['_', FST_(1:end-3), 'out'];
    end
    ...
    if ( length( le_input_files_work ) == 5 ) %& exist('Check_AeroDyn15') & exist('Check_ElastoDyn') & exist('Check_ServoDyn') & exist('Check_FST') ) %#ok<AND2>
    
        % fprintf('** Modified files (in the working dir. + internal dir.) : \n')
...
if ( exist('results_extra') > 0)
    rmdir('results_extra','s');
end
    ...
    % present in the directory or the user decided to modify them (selectModififyConfigFile = 2 / yes or 1)
            ...
            % Check if the compiled python program is present, otherwise
            % stop the program
            if ( exist('internalPython/dist/fast_input_file/fast_input_file') < 2)
    
            fprintf('* * * WARNING : the Python script should be compiled using ''pyinstaller'' \n');
            fprintf('to avoid librairies conflicts with the usual Python system call.  \n');
            fprintf('Please set the executable file ''fast_input_file'' in the folder : \n');
            fprintf('<./internalPython/dist/fast_input_file/ [fast_input_file] > * * * \n\n');
    
            fprintf('\n Please restart or continue without Python \n\n');
    ...
    if (ModifyOF_file_check > 0 & UsePython == 1) %#ok<OR2>
        ...
        for ( cnt_Input = 1:length( InputSettings(:,1)) ) %#ok<NO4LP>
            ...
            % -ServoDyn
            run_python_fast_input_file = sprintf("./dist/fast_input_file/fast_input_file  %s '%s' '%s' %s 0 ", ServoDyn_modified, [LCOS_rootFolder, '/internalPython/', ServoDyn_modified], InputSettings(cnt_Input,1), InputSettings(cnt_Input,2));
            system(run_python_fast_input_file);
    
            % -AeroDyn15
            run_python_fast_input_file = sprintf("./dist/fast_input_file/fast_input_file  %s '%s' '%s' %s 0 ", AeroDyn15_modified, [LCOS_rootFolder, '/internalPython/', AeroDyn15_modified], InputSettings(cnt_Input,1), InputSettings(cnt_Input,2));
            system(run_python_fast_input_file);
    
            % -ElastoDyn
            run_python_fast_input_file = sprintf("./dist/fast_input_file/fast_input_file  %s '%s' '%s' %s 0 ", ElastoDyn_modified, [LCOS_rootFolder, '/internalPython/', ElastoDyn_modified], InputSettings(cnt_Input,1), InputSettings(cnt_Input,2));
            system(run_python_fast_input_file);
    
            % -FAST_FstFile
            run_python_fast_input_file = sprintf("./dist/fast_input_file/fast_input_file  %s '%s' '%s' %s 0 ", FAST_FstFile_modified, [LCOS_rootFolder, '/internalPython/', FAST_FstFile_modified], InputSettings(cnt_Input,1), InputSettings(cnt_Input,2));
            system(run_python_fast_input_file);
    
            % -InflowWind
            run_python_fast_input_file = sprintf("./dist/fast_input_file/fast_input_file  %s '%s' '%s' %s 0 ", InFlow_modified, [LCOS_rootFolder, '/internalPython/', InFlow_modified], InputSettings(cnt_Input,1), InputSettings(cnt_Input,2));
            system(run_python_fast_input_file);
        ...
        % -FAST_FstFile
        run_python_fast_input_file = sprintf("./dist/fast_input_file/fast_input_file  %s '%s' '%s' %s 0 ", FAST_FstFile_modified, [LCOS_rootFolder, '/internalPython/', FAST_FstFile_modified], 'SumPrint', 'True');
        system(run_python_fast_input_file);
    
        fprintf(' End of the modifications. \n');
    
        %cd ..
    
        % Clear copied '_*' input file from the OpenFAST dir.
      ...
      %  delete('*.bts')
        ...
        % Move each modified file to the working dir. + the original openFAST directory
    
        % ([v1.6.1b] except the ElastoDyn whose IC on the pitch may be modified again)
    
        % Move first to the working directory then copy to the original openFAST directory (v1.8.3)
        movefile([internalPython_Path, AeroDyn15_modified], [path, 'OFastFiles/'])
        %
        % (v1.8) the ElastoDyn will also move and will be copied again from
        % its orginal directory for the pitch matching
        movefile([internalPython_Path, ElastoDyn_modified], [path, 'OFastFiles/'])
        %
        movefile([internalPython_Path, ServoDyn_modified], [path, 'OFastFiles/'])
        %
        movefile([internalPython_Path, FAST_FstFile_modified], [path, 'OFastFiles/'])
        %
        movefile([internalPython_Path, InFlow_modified], [path, 'OFastFiles/'])
    
        % copy these files to the original openFAST directory
        copyfile([path, 'OFastFiles/'], [LCOS_rootFolder, FAST_InputPath(2:end)])
      ...
      %  delete('*.bts')
        ...
        % if the 5 files are present in the working dir. -> copy these
        % files to the original openFAST directory (v1.8.3)
        if ( length( le_input_files_work ) == 5 )
    ...
    if ( OnlyReadingInput == 0 & UsePython == 1) % set pitch angle when SelectTrajectoryComputation == 1
        ...
        fprintf('Initialize the Simulink file - set the pitch I.C. in ''ElastoDyn'' file) to %d degrees \n', floor(PitchBlade_0) )
        ...
        PitchBlade_0_str = sprintf("%d", floor(PitchBlade_0));
        ...
        copyfile(full_InFlow_modified ,[internalPython_Path, InFlow_modified]);
        ...
        for ( cnt_Input = 1:length( ICPitchModification(:,1)) ) %#ok<NO4LP>
            ...
            % -ElastoDyn
            run_python_fast_input_file = sprintf("./dist/fast_input_file/fast_input_file  %s '%s' '%s' %s  0 ", ElastoDyn_modified, [LCOS_rootFolder, '/internalPython/', ElastoDyn_modified], ICPitchModification(cnt_Input,1), ICPitchModification(cnt_Input,2));
            system(run_python_fast_input_file);
    
        end
    
        % -AeroDyn15
        run_python_fast_input_file = sprintf("./dist/fast_input_file/fast_input_file  %s '%s' '%s' %s 0 ", AeroDyn15_modified, [LCOS_rootFolder, '/internalPython/', AeroDyn15_modified], "BlOutNd", ControlledNode_str);
        system(run_python_fast_input_file);
    
        % -Servodyn
        run_python_fast_input_file = sprintf("./dist/fast_input_file/fast_input_file  %s '%s' '%s' %s 0 ", ServoDyn_modified, [LCOS_rootFolder, '/internalPython/', ServoDyn_modified], "Ptch_Cntrl", IPC_mode_str);
        system(run_python_fast_input_file);
        ...
        fprintf("WARNING: no modification has been made in the input files! \n")
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Check of files equality (working dir. and internal dir.) (v1.8.3)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    mkdir temp
    
    % InFlow
    copyfile( [path, 'OFastFiles/', InFlow_modified], './temp/test_A.m')
    
    copyfile( [LCOS_rootFolder, FAST_InputPath(2:end), InFlow_modified], './temp/test_B.m')
    
    cd temp
    [status,result] = system('cmp test_A.m  test_B.m');
    
    if ( status == 1)
        result
    end
    
    result_cmp = result_cmp + status;
    
    delete test_A.m test_B.m
    cd ..
    
    % ElastoDyn
    copyfile( [path, 'OFastFiles/', ElastoDyn_modified], './temp/test_A.m')
    
    copyfile( [LCOS_rootFolder, FAST_InputPath(2:end), ElastoDyn_modified], './temp/test_B.m')
    ...
    % AeroDyn15
    copyfile( [path, 'OFastFiles/', AeroDyn15_modified], './temp/test_A.m')
    
    copyfile( [LCOS_rootFolder, FAST_InputPath(2:end), AeroDyn15_modified], './temp/test_B.m')
    ...
    % ServoDyn
    copyfile( [path, 'OFastFiles/', ServoDyn_modified], './temp/test_A.m')
    
    copyfile( [LCOS_rootFolder, FAST_InputPath(2:end), ServoDyn_modified], './temp/test_B.m')
    ...
    % FAST_FstFile
    copyfile( [path, 'OFastFiles/', FAST_FstFile_modified], './temp/test_A.m')
    
    copyfile( [LCOS_rootFolder, FAST_InputPath(2:end), FAST_FstFile_modified], './temp/test_B.m')
    ...
    if ( result_cmp == 0)
        fprintf('OK with internal files checking... \n')
    else
        fprintf('Problem, Abort the program');
    
        pa
    end

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % End of checking (working dir. and internal dir.) (v1.8.3)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( ( SelectTrajectoryComputation == 0 || SelectTrajectoryComputation == 2 ) & BuildRef_check > 0 ) %#ok<AND2>
        ...
        SimulationKernel_vB; %  'from v1.6.0 : WindTurbineSlx_v2 invokes directly ProcessingSlxData_v5_cell'
        ...
        if (SelectTrajectoryComputation == 0)
            ...
            if (ComputeSurface == 1)
                ...
                [Stat_vector, IndexSurfaceComputation] = ComputeLiftSurface_v2c (NumBlNds, IndexSurfaceComputation, 0, NodeLift_Index_cntrl, Results, ResultsLiftdata, path, Stat_vector, 0, 0 , 0 );
    ...
    size_wind_speed = size( wind_speed_x );
    
    % Update the wind profile (new in v1.8.3)
    for ind_wind = 1 : size_wind_speed(1)
    
        cd(LCOS_rootFolder)
    
        WriteWindProfile_v2(wind_speed_t(ind_wind,:), wind_speed_x(ind_wind,:), FAST_InputPath, LCOS_rootFolder, PwrLaw);
    
        fprintf('==== Using Wind Profile %d of %d ==== \n', ind_wind, size_wind_speed(1));
    
        pause(5)
    
        % Define the starting time of the plots
        if ( exist('Start_time_plotting','var') == 1)
            beg = Start_time_plotting;
        else
            fprintf('Warning: ''Start_time_plotting'' is not defined - starting plotting from the origin \n');
            beg = 1;
        end
    ...
    if ( ForceKeepOutData == 0 )
        ...
        movFILE_out = [FAST_InputPath(1:end), Check_FAST_OutFile];
    
        movefile(movFILE_sum, [path, Check_FAST_SumFile ] );
    
        if ( KeepOUTfile == 1)
            movefile(movFILE_out, [path, Check_FAST_OutFile ] );
        end
