    ...
    %%
    %%
    %%  LCO/St - Lift Control OpenFAST & Stabilization / Main Module - Dec 2023 - Oct. 2024
    %%                                   v1.8.2
    ...
    fprintf('\n\n === LCO/St - Lift Control OpenFAST & Stabilization / Main Module - v1.8.2 (Oct. 2024) === \n\n')
    ...
    if (ModifyOF_file_check > 0 & UsePython == 1) %#ok<OR2>
        ...
        for ( cnt_Input = 1:length( InputSettings(:,1)) ) %#ok<NO4LP>
            ...
            % -InflowWind
            run_python_fast_input_file = sprintf("python3 fast_input_file.py %s '%s' '%s' %s 0 ", InFlow_modified, [LCOS_rootFolder, '/internalPython/', InFlow_modified], InputSettings(cnt_Input,1), InputSettings(cnt_Input,2));
            system(run_python_fast_input_file);
