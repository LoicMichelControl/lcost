...
function [Stat_vector, IndexSurfaceComputation] = ComputeLiftSurface_v2c (NumBlNds, ...
                                                                         IndexSurfaceComputation, ...
                                                                         SurfaceMaxTime, ...
                                                                         NodeLift_Index_cntrl, ...
                                                                         Results, ...
                                                                         ResultsLiftdata, ...
                                                                         path, ...
                                                                         Stat_vector, ...
...
IndexSurfaceComputation = IndexSurfaceComputation + 1;
 ...
 %   IndMaxSubTime = max( find ( Time_x <= SurfaceMaxTime ) ); %#ok<MXFND>
    ...
    fprintf('\n ----- Computing the lift surface (takes few minutes) - some statistics : ');
...
file_name_SC_Fl_fig = sprintf('SurfaceLift_Fl_pitch_%f_windSpeed_%f.fig', PitchBlade_0, Results.Wind1VelX_out(end)  );

  file_name_SC_FlFy = sprintf('SurfaceLift_FlFy_pitch_%f_windSpeed_%f.png', PitchBlade_0, Results.Wind1VelX_out(end)  );
file_name_SC_FlFy_fig = sprintf('SurfaceLift_FlFy_pitch_%f_windSpeed_%f.fig', PitchBlade_0, Results.Wind1VelX_out(end)  );  
...
cd(path)
    ...
    hold on
    plot(Time_x(ReducedTimeVector), LiftCntrlLine(ReducedTimeVector), '--m', 'linewidth', 2)
    ...
    set(gcf,'Color','w');
     ...
     saveas(gcf,file_name_SC_Fl_fig);
    ...
    hold on
    plot(Time_x(ReducedTimeVector), LiftCntrlLine(ReducedTimeVector), '--m', 'linewidth', 3)
    ...
    set(gca,'FontSize',ft_size);
    set(gcf,'Color','w');
    saveas(gcf,file_name_SC_FlFy);
    saveas(gcf,file_name_SC_FlFy_fig);
