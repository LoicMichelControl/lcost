    ...
    if ( plot_5 == 1 ) % plot 5 - Fl#1 - Fl#2 - Fl#3
        ...
        hold on
        plot ( Results(uu).Time(beg:end), Results(uu).B1N1Fl_out(beg:end), 'b', 'LineWidth',2 )
        plot ( Results(uu).Time(beg:end), Results(uu).TrajectoryReference(beg:end), '--r' , 'LineWidth',3 )
        plot ( Results(uu).Time(beg:end), Results(uu).Avg_B1N1Fl_out(beg:end), 'c', 'LineWidth',2 )
        grid on
        grid on
        ylabel('Fl 1 [N]', 'interpreter', 'latex')
        legend('normal output', 'reference', 'averaged output', 'interpreter', 'latex');
        set(gcf,'Color','w');
        set(gca,'FontSize',ft_size);
     %   title(title_graph)
        subplot(3,1,2)
        ...
        grid on
        grid on
        ylabel('Fl 2 [N]', 'interpreter', 'latex')
        legend('normal output', 'reference', 'averaged output', 'interpreter', 'latex');
        set(gcf,'Color','w');
        set(gca,'FontSize',ft_size);
        subplot(3,1,3)
        ...
        ylabel('Fl 3 [N]', 'interpreter', 'latex')
        legend('normal output', 'reference', 'averaged output', 'interpreter', 'latex');
        set(gcf,'Color','w');
        set(gca,'FontSize',ft_size);
        if (save_plot == 1)
    ...
    if ( plot_6 == 1 ) % plot 6 - AVG Fl#1 + pitch angle
        ...
        grid on
        ylabel('AVG Fl#1')
        legend('simulated', 'reference (base)', 'auto ref.');
        set(gcf,'Color','w');
        set(gca,'FontSize',ft_size);
     %   title(title_graph)
        subplot(2,1,2)
